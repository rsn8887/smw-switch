...
void loadImGuiCfg(){
	FILE *f = fopen("ux0:data/smw/imgui.cfg", "rb");
	if (f){
		char str[256];
		fread(str, 1, 256, f);
		fclose(f);
...
void ImGui_callback() {
		...
		if (ImGui::BeginMainMenuBar()){
			...
			if (ImGui::BeginMenu("Extra")){
				...
				if (ImGui::MenuItem("Save settings")){
					FILE *f = fopen("ux0:data/smw/imgui.cfg", "wb+");
					char str[256];
	...
	if (credits_window){
		...
		ImGui::TextColored(ImVec4(255, 255, 0, 255), "SuperMarioWar v.1.0");
		...
		ImGui::Text("nop90 for the 3DS port used as base for this port");
		ImGui::End();
	}
